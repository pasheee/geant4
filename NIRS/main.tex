...
Допант мишени                          & Cd ($0.1\%$) или Gd ($0.1\%$) \\
...
\end{axis}
\end{tikzpicture}
\caption{Спектр кинетической энергии позитронов в реакции IBD при условии $N_{fired}\ge 3$ ($76.1\%$ от $10^5$ событий).}
...
\end{axis}
\end{tikzpicture}
\caption{Распределение времени жизни тепловых нейтронов до захвата для Cd и Gd (массовая доля $0.1\%$) при условии регистрации $N_{fired}\ge 3$.}
\label{fig:lifetime}
\end{figure}
...
\end{subfigure}
\caption{Сравнение отклика детектора на захват теплового нейтрона для допантов Cd и Gd при массовой доле $0.1\%$ и условии $N_{fired}\ge 3$ ($10^5$ событий каждой конфигурации). Слева~--- гистограмма $N^{tot}_{pe}$ в логарифмическом масштабе. Справа~--- кривая выживания: кривая Gd лежит выше Cd на всём диапазоне световых выходов.}
...
Одиночный фотоэлектрон не пригоден в качестве условия регистрации, поскольку каждый ФЭУ имеет тёмновой ток --- термоэмиссию электронов с фотокатода в отсутствие оптического сигнала. Характерный тёмновой счёт составляет $r_{dark}\approx 1$~кГц на ФЭУ, что при пороге в один сработавший фотоумножитель даёт суммарный фоновый поток $N_{PMT}\cdot r_{dark}=38\times 10^3$~Гц --- на несколько порядков выше любой разумной скорости счёта сигнала от IBD.
...
1) Учёт углового распределения позитронов в модели.

2) Оценка ожидаемой скорости счёта реакций IBD на Калинской АЭС с учётом полученных эффективностей регистрации ($\varepsilon_{prompt}\cdot\varepsilon_{delayed}\approx 56\%$ для Gd).
